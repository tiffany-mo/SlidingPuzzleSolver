...
public class MazeState extends State {
    ...
    public MazeState(String mapString) {
        ...
        this.spaces = new boolean[this.height][this.width];
    ...
    public List<Action> getActions() {
        ...
        if (this.row > 0 && this.spaces[this.row-1][this.col]) {
            actions.add(new Action(
                "up", 1,
                new MazeState(this.toString(this.row - 1, this.col))
            ));
        }
        if (this.row < this.height - 1 && this.spaces[this.row + 1][this.col]) {
            actions.add(new Action(
                "down", 1,
                new MazeState(this.toString(this.row + 1, this.col))
            ));
        }
        if (this.col > 0 && this.spaces[this.row][col - 1]) {
            actions.add(new Action(
                "left", 1,
                new MazeState(this.toString(this.row, this.col - 1))
            ));
        }
        if (this.col < this.width - 1 && this.spaces[this.row][this.col + 1]) {
            actions.add(new Action(
                "right", 1,
                new MazeState(this.toString(this.row, this.col + 1))
            ));
